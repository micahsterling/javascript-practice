...
export function monthRate(ratePerHour, discount) {
  const discountRate = (dayRate(ratePerHour) * 22 * discount);
  const month = (ratePerHour * 8 * 22);
  return Math.ceil(dayRate(ratePerHour) - dayRate(ratePerHour)-discount);
}
...
export function daysInBudget(budget, ratePerHour, discount) {
  ...
  const discountRate = dayRate * discount
  return Math.floor(budget / (dayRate - discountRate));
}
